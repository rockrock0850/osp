...
(function ($) {
    ...
    $.fn.gantt = function (options) {
        ...
        var scales = ["days", "weeks", "months", "seasons", "year"];
        //Default settings
        var settings = {
            source: null,
            itemsPerPage: 10,
            months : ["一月", "二月", "三月", "四月", "五月", "六月", "七月", "八月", "九月", "十月", "十一月", "十二月"],
			seasons : ["第一季", "第二季", "第三季", "第四季"],
			dow : ["日", "一", "二", "三", "四", "五", "六"],
            startPos: new Date(),
            navigate: "scroll",
            scale: "days",
		...
		// return the month for the season
        ...
        var core = {
            ...
            render: function (element) {
				...
				var $dataPanel = $('div[class="dataPanel"]');
				
				switch(settings.scale) {
					case 'seasons':
					case 'year':
						$dataPanel.css('background-image', 'url(assets/plugins/jquery-gantt/img/grid-large.png)');
						$dataPanel.css('background-repeat', 'repeat');
						$dataPanel.css('background-position', '48px 24px');
						break;
					default:
						$dataPanel.css('background-image', 'url(assets/plugins/jquery-gantt/img/grid.png)');
						$dataPanel.css('background-repeat', 'repeat');
						$dataPanel.css('background-position', '24px 24px');
						break;
				}
            ...
            rightPanel: function (element, leftPanel) {
                ...
                switch (settings.scale) {
                    ...
                    case 'months':
                        ...
                        break;
					case "seasons":
                        range = tools.parseSeasonsRange(element.dateStart, element.dateEnd);

                        var year = range[0].getFullYear();
						var totalSeason = 0;
						
						var hasSeason1 = false;
						var hasSeason2 = false;
						var hasSeason3 = false;
						var hasSeason4 = false;
	
                        for (var i = 0; i < range.length; i++) {
                            var rday = range[i];
							var season = rday.getSeason();
							var seasonTxt = '';
                            ...
                            if (rday.getFullYear() !== year) {
                                yearArr.push(
                                    ('<div class="row header year" style="width: ' 
									+ tools.getSeasonCellSize() * seasonInYear 
									+ 'px;"><div class="fn-label">' 
									+ year + '</div></div>'));
                            ...
                            if(season == 0 && !hasSeason1) { // 第一季
								hasSeason1 = true;
								seasonFlag = true;
							} else if(season == 1 && !hasSeason2) {// 第二季
								hasSeason2 = true;
								seasonFlag = true;
							} else if(season == 2 && !hasSeason3) {// 第三季
								hasSeason3 = true;
								seasonFlag = true;
							} else if(season == 3 && !hasSeason4) {// 第四季
								hasSeason4 = true;
								seasonFlag = true;
							}
										
							if(seasonFlag) {
								seasonTxt = settings.seasons[season];
								seasonInYear++;
								...
								seasonArr.push(
									...
									+ '" offset="' + totalSeason * tools.getSeasonCellSize() 
									+ '" repdate="' + rday.genRepDate() + '" style="width: ' 
									+  tools.getSeasonCellSize()
									+ 'px;"><div class="fn-label">' 
                            		+ seasonTxt // 季份資料 
                            		+ '</div></div>');
                        ...
                        var dataPanel = core.dataPanel(element, totalSeason * tools.getSeasonCellSize());
						
                        // Append panel elements
                        dataPanel.append(yearArr.join(""));
                        dataPanel.append(seasonArr.join(""));
                        // dataPanel.append($('<div class="row"/>').html(seasonArr)); // 月/日

                        break;   	
					case 'year':
						range = tools.parseYearsRange(element.dateStart, element.dateEnd);
						...
						for (var i = 0; i < range.length; i++) {
							...
							if (rday.getFullYear() !== year) {
								yearArr.push(
									('<div class="row header year" id="' 
										 + year 
										 + '" count="' + yearCount + '" style="width: '
										 + (tools.getCellSize() * 2) + 'px;">'
										 +'<div class="fn-label">'
										 + year
										 + '</div></div>'));
								year = rday.getFullYear();
						...
						yearArr.push(
							('<div class="row header year" id="' 
								 ...
								 + year
								 + '</div></div>'));				
						var dataPanel = core.dataPanel(element, yearCount * 2 * tools.getCellSize());
                            ...
                            .append($('<div class="nav-slider-left" />')
                                ...
                                .append($('<span role="button" class="nav-link nav-prev-week"/>')
                                    ...
                                    .click(function () {
                                        ...
                                        } else if (settings.scale === 'seasons') {
                                            core.navigateTo(element, tools.getCellSize() * 6);
                                        } else if (settings.scale === 'year') {
                                            core.navigateTo(element, tools.getCellSize() * 6);
                                        }
                                    }))
                        ...
                        $.each(entry.values, function (j, day) {
                            ...
                            switch (settings.scale) {
								...
								case "seasons":
                                    ...
                                    var from = $(element).find("#dh-" + dtFrom.getFullYear() + '-' + dtFrom.getSeason());
                                    var cFrom = from.attr("offset");
											
									var to = $(element).find("#dh-" + dtTo.getFullYear() + '-' + dtTo.getSeason());
                                    var cTo = to.attr("offset");
                                    var dl = tools.getSeasonRange(dtFrom.getFullYear() + '-' + dtFrom.getSeason(), dtTo.getFullYear() + '-' + dtTo.getSeason());
								...
								case 'year':								
									...
									var start = $('div[id="' + dtFrom.getFullYear() + '"]').attr('count');
									var end = $('div[id="' + dtTo.getFullYear() + '"]').attr('count');
										
									var cFrom = (parseInt(end, 10) - parseInt(start, 10) + 1);
									var dl = cFrom * 2;
                                    ...
                                    _bar.css({ 'margin-top': top, 'margin-left': tools.getCellSize() * ( start - 1 ) * 2});
									
									// datapanel.append(tools.processYearBar(_bar, dtFrom, dtTo));
									datapanel.append(_bar);
									break;
                                // **Days**
                                default:
            ...
            zoomInOut: function (element, val) {
                core.waitToggle(element, true, function () {
                    ...
                    if (settings.scale === "hours" && scaleSt >= 13) {
                        ...
                        headerRows = 2;
                        scaleSt = 14;
					} else if (settings.scale === "year" && zoomIn) {
						scale = "seasons";
						headerRows = 2;
						scaleSt = 14;
					} else if (settings.scale === "seasons" && zoomIn) { // 放大/拉近
                        scale = "months";
                        headerRows = 2;
                        scaleSt = 14;
                    } else if(settings.scale === "seasons" && !zoomIn) {
						scale = "year";
						headerRows = 1;
						scaleSt = 1;
					}
        ...
        // =================
        var tools = {		
			processYearBar: function(bar, from, to) {
				var frMonth = from.getMonth() + 1;
				var enMonth = to.getMonth() + 1;

				if(frMonth != 1) {
					var cellSize = frMonth < 6 ? tools.getCellSize() : tools.getYearCellSize();
					var modifyWidth = cellSize * ( (12 - frMonth) / 12);
					var width = parseInt(bar.css('width').split('px')[0], 10);
					var marginLeft = parseInt(bar.css('margin-left').split('px')[0], 10);
					
					bar.css('margin-left', marginLeft + modifyWidth).css('width', width - modifyWidth);
				}
				
				if(enMonth != 12) {
					var cellSize = enMonth < 6 ? tools.getCellSize() : tools.getYearCellSize();
					var modifyWidth = cellSize * ( (12 - enMonth) / 12) - 2;
					var width = parseInt(bar.css('width').split('px')[0], 10);
					
					bar.css('width', width - modifyWidth);
				}
				
				return bar;
			},
			
			getSeasonRange: function(from, to) {
				var fYear = parseInt(from.split('-')[0], 10);
				var fSeason = parseInt(from.split('-')[1], 10) + 1;
				
				var eYear = parseInt(to.split('-')[0], 10);
				var eSeason = parseInt(to.split('-')[1], 10) + 1;
				
				return ((eYear - fYear) * 4 + 1) + (eSeason - fSeason);
			},
            ...
            getMaxDate: function (element) {
                ...
                switch (settings.scale) {
					...
					case "seasons":
                        ...
                        break; 
					case 'year':
						maxDate = new Date(maxDate.getFullYear() + element.scaleStep, maxDate.getMonth(), 1);
						break;
                    default:
                        maxDate.setHours(0);
            ...
            getMinDate: function (element) {
                ...
                switch (settings.scale) {
					...
					case "seasons":
                        ...
                        break; 
					case 'year':
						minDate = new Date(minDate.getFullYear() - element.scaleStep, minDate.getMonth(), 1);
						break;
                    default:
                        minDate.setHours(0);
			...
			// Return an array of Date objects between a range of seasons between `from` and `to`
            parseSeasonsRange: function (from, to) {
                var current = new Date(from);
                ...
                do {
                    ret[i++] = new Date(current.getFullYear(), current.getMonth(), 1);
                    current.setMonth(current.getMonth() + 1); // 把起迄日拉出來，算出中間間隔的月份
                } while (current.getTime() <= to.getTime());

                return ret;
            },
			
			// Return an array of Date objects between a range of years
			// between 'from' and 'to'
			parseYearsRange : function (from, to) {
				var current = new Date(from);
				var end = new Date(to);
            ...
            genId: function (ticks) {
                ...
                switch (settings.scale) {
                    ...
                    case "seasons":
                        return t.getFullYear() + "-" + t.getSeason();
					case "year":
			...
			getSeasonCellSize: function() {
				return settings.largeCellSize;
			},
			
			getYearCellSize: function() {
				return settings.largeCellSize;
			},
        ...
        this.each(function () {
            ...
            */
            ...
            switch (settings.scale) {
				...
				case "seasons": this.headerRows = 2; this.scaleStep = 14; break;
				case "year": this.headerRows = 1; this.scaleStep = 2; break;
