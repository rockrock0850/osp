require(['flip', 'service', 'osp-util', 'common-util', 'cookie', 'form2object',  
         'jquery.dataTables', 'htmlComponet', 'DT_bootstrap'], 
	function (flip, service) {
		document.title = 'OSP | 部門待處理';
		service.init();
...
(function() {
	define('service', ['flip', 'flow-main-service', 'htmlComponet', 'osp-util'], 
		function(flip, flowMainService) {
			...
			var setFormPositionEvent = function() {
				$('.page-sidebar').click(function() {
					if(!$('body').hasClass('page-sidebar-closed')) {
						$.setMenuOpen($('form#searchForm'), 'dept_todo');
					}else{
						$.setMenuClosed($('form#searchForm'), 'dept_todo');
					}
				});
			};
			...
			var setResultTable = function() {
				...
				var settings = {
					httpMethod: 'POST', 
					URL: contextPath + '/workshop/dept/get-dept-todo-info.action',
					dataType: 'html',
				...
				$table.DataTable({
					...
					columnDefs: [ // 欄位設定
						...
						}},
						{
							...
							defaultContent: "<button id='assignButton' class='btn btn-primary btn-sm'>認領</button>"
						},
			...
			var setTableButtonClickEvent = function() {
				var $dataTable = $('table#resultTable');
				
				// 修改案件類別設定
				$dataTable.unbind('click').on('click', '#contentButton', function() {
					// 處理click event
					var $row = $(this).closest('tr');// closest(): 找到最接近的指定元素
					var flowId = $row.find('.FLOW_ID').text();
					var orderMId = $row.find('.ORDER_M_ID').text();
					var sourceSysId = $row.find('.SOURCE_SYS_ID').text();
					var orderTypeId = $row.find('.ORDER_TYPE_ID').text();
