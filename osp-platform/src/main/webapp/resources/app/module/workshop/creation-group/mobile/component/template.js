require(['service', 'flip', 'DT_bootstrap', 'jquery.dataTables'], 
	function (service) {
		...
		$('button#sendEMailButton').unbind('click').click(function() {
			if ($.hasChecked($('table#msisdnTable').find('input:checkbox'), '035')) {
				var settings = {
					httpMethod: 'POST', 
	...
	define('service', ['jquery', 'flow-main-service', 'htmlComponet', 'small-modal'], 
		function($, flowMainService) {
			    ...
			    if(!$.isEmptyObject(formData)) {
				    $.ajax({
				        ...
				        error: function (xhr, ajaxOptions, thrownError) {
							var content = '<center><label>' + xhr.responseText + '</label></center>';
							var buttons = {
								cancelButton: {
									value: '確認'
								}
							}
				        	$.messageModal('錯誤', content, buttons);
				        }
				    });
			    }else{
		        	$.warningMsg('006');
			...
			var isBlank = function () {
				...
				var corpPicTaxId = $('div#creationFile').find('input[name=corpPicTaxId]').val();
